...
describe("loadConfig", () => {
  it("throws Config error when REDASH_URL is missing", () => {
    ...
    expect(() => loadConfig()).toThrow(
      expect.objectContaining({ kind: RedashErrorKind.Config, message: expect.stringContaining("REDASH_URL") }),
    );
  });
  ...
  it("throws Config error when REDASH_API_KEY is missing", () => {
    ...
    expect(() => loadConfig()).toThrow(
      expect.objectContaining({ kind: RedashErrorKind.Config, message: expect.stringContaining("REDASH_API_KEY") }),
    );
  });
  ...
  it("throws Config error for malformed REDASH_EXTRA_HEADERS", () => {
    ...
    expect(() => loadConfig()).toThrow(
      expect.objectContaining({ kind: RedashErrorKind.Config, message: expect.stringContaining("REDASH_EXTRA_HEADERS") }),
    );
  });
